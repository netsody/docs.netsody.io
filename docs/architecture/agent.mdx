...
Manages the creation and lifecycle of virtual network interfaces used to connect the system to Netsody overlay networks. These interfaces (implemented as [TUN devices](https://en.wikipedia.org/wiki/TUN/TAP)) expose regular IP connectivity to local applications, while all traffic is transparently processed by the agent according to the overlay configuration.

- **Resource Coordination**
Installs system-level forwarding entries for resource subnets via the Netsody network. This allows access to remote physical networks that are otherwise unreachable, by having Netsody nodes act as gateways. Access to each resource still requires a matching policy.
