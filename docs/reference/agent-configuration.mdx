...
:::note
The `[prometheus]` section is only available in agent builds with the `prometheus` Cargo feature enabled. It is not part of the default build.
:::

When configured, the agent pushes its metrics every five seconds to the Prometheus Pushgateway at `url`, using HTTP basic authentication with the given credentials. All three fields are required within the section:

- `url` (string): URL of the Prometheus Pushgateway.
- `user` (string): user for HTTP basic authentication.
- `pass` (string): password for HTTP basic authentication.
